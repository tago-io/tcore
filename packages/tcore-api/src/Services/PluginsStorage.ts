...
export async function getPluginStorageItem(pluginID: string, key: string) {
  const response = await invokeDatabaseFunction("getPluginStorageItem", pluginID, key);
  return response;
}
 ...
 */
export async function setPluginStorageItem(pluginID: string, key: string, value: any) {
