...
async function storeQOSMessage(device_id: string, model: IQOSMessage) {
  ...
  if (database === null) {
    ...
    return pluginStorage.set(`${database.id}_QOS_${model.topic}_${model.messageId}`, model);
  }
}

async function delQOSMessage(device_id: string, topic: string, message_id: string) {
  const database = await core.getDeviceInfo(device_id).catch(() => null);
...
async function getQOSMessage(device_id: string, topic: string): Promise<IQOSMessage[]> {
  const database = await core.getDeviceInfo(device_id).catch(() => null);
  ...
  if (database === null) {
    ...
    const topicKeys = storedKeys.filter((key) => key?.includes?.(`${database.id}_QOS_${topic}_`));
...
export { storeQOSMessage, getQOSMessage, delQOSMessage, IQOSMessage };
