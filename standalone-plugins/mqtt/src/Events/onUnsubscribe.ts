...
async function onUnsubscribe(client: ITagoIOClient, packet: IUnsubscribePacket & { qos: number }) {
  client.topics = [...(client.topics || []).filter((x) => !packet.unsubscriptions.some((y) => y === x))];
