...
async function onPublish(connection: Connection, client: ITagoIOClient, packet: PublishPacket) {
  if (!client.channel) {
  ...
  if (client.payload_type === "hex") {
    payload = Buffer.isBuffer(packet.payload) ? packet.payload.toString("hex") : String(packet.payload);
  } else {
    payload = String(packet.payload);
  }

  const scope: { topic: string; payload: any; qos: number; messageId?: string } = {
    topic: packet.topic,
    payload,
    qos: packet.qos,
  };
  ...
  actionList = actionList.filter((action) => action?.type?.includes?.("mqtt-action-trigger"));

  const canTrigger = (topics) => Array.isArray(topics) && topics.find((topic) => channelRegex(topic).test(scope.topic));
  for (const action of actionList) {
    if (canTrigger(action.trigger?.topic)) {
      await core.triggerAction(action.id, { ...scope, device: client.device.id });
    }
  }
