...
const NetworkService = new ServiceModule({
  ...
  configs: [
    {
      name: "Port",
      tooltip: "Port used by the integration to receive connection from Actility",
      icon: "cog",
      field: "port",
      type: "number",
      required: true,
    ...
    {
      name: "Tunnel Interface Key",
      tooltip: "Tunnel Interface Key from your integration on Actility portal",
      icon: "cog",
      field: "tunnel_key",
      type: "string",
      required: true,
    ...
    },
    {
      ...
      tooltip: "Enter an authorization code to be used at Actility",
      icon: "cog",
...
const action = new ActionTypeModule({
  id: "actility-downlink-trigger",
  name: "Actility Downlink-Action",
  option: {
    description: "Send a downlink to a Actility LoraWAN device",
    name: "Downlink to Actility Device",
  ...
  const startServer = () => {
    return new Promise((resolve, reject) => {
      ...
      server = app.listen(configParams.port, () => {
        console.info(`Actility-Integration started at port ${configParams.port}`);
        resolve(true);
      });
