  ...
  if (bytes.length === 11) {
    ...
    for (let i = 0; i < bytes.length; ) {
      ...
      data.push({
        ...
        value: parseFloat(((((bytes[i + 3] << 24) >> 16) | bytes[i + 4]) / 100).toFixed(2)),
        unit: "°C",
      });
      //Humidity
      data.push({
        variable: `entry_${i / 11}_Hum`,
        value: parseFloat((((bytes[i + 5] << 8) | bytes[i + 6]) / 10).toFixed(2)),
        unit: "%",
      });
      //Unix Time Stamp
      data.push({
        variable: `entry_${i / 11}_unix_time`,
        value: ((bytes[i + 7] << 24) | (bytes[i + 8] << 16) | (bytes[i + 9] << 8) | bytes[i + 10]) * 1000,
      });
      i += 11;
    }
    return data;
    ...
    return [
      {
        ...
        value: "Parser Error: payload length does not match, must have 11 bytes or it's multiples.",
      },
    ];
  }
}
 ...
 */
export default async function parserLAQ4(payload: IDeviceDataCreate[]): Promise<IDeviceDataCreate[]> {
  ...
  if (data) {
    ...
    payload = payload
      .concat(Decoder(bytes))
      .map((x) => ({ variable: x.variable.toLowerCase(), value: x.value, unit: x.unit, serie }));
  }
