...
import ttnParser from "./Services/ttnParser.ts";
...
const action = new ActionTypeModule({
  id: "ttn-downlink-trigger",
  name: "TTN/TTI Downlink-Action",
  option: {
    description: "Send a downlink to a LoRaWAN TTN/TTI device",
    name: "Downlink to TTN/TTI Device",
  ...
  const startServer = () => {
    return new Promise((resolve, reject) => {
      ...
      server = app.listen(configParams.port, () => {
        console.info(
          `TTN/TTI-Integration started at port ${configParams.port}`,
        );
        resolve(true);
      });
