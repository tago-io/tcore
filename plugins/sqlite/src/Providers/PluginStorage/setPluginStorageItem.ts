 ...
 */
async function setPluginStorageItem(pluginID: string, data: IDatabaseSetPluginStorageData): Promise<void> {
  ...
  await connection.insert(object).into("storage").onConflict("key").merge(["key", "value", "type"]);
}
