import { IAnalysisList, IDatabaseAnalysisListQuery } from "@tago-io/tcore-sdk/types";
 ...
 */
async function getAnalysisList(query: IDatabaseAnalysisListQuery): Promise<IAnalysisList> {
  const { page, amount, orderBy, fields, filter } = query;
  const pagination = applyQueryPagination(page, amount);

  const knexQuery = knexClient
    .select(fields)
    .from("analysis")
  ...
  if (filter.binary_path) {
    knexQuery.where("binary_path", "like", replaceFilterWildCards(filter.binary_path));
  }
  if (filter.file_path) {
    knexQuery.where("file_path", "like", replaceFilterWildCards(filter.file_path));
  }
  if ("active" in filter) {
